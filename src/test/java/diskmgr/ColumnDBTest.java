...
public class ColumnDBTest extends TestDriver implements GlobalConst{
        ...
        for (int i = 0; i < 6 && status == OK; ++i) {
            ...
            if (status == OK) {
                ...
                for (int j = 0; j < 20 && status == OK; ++j) {
                    ...
                    try {
                        SystemDefs.JavabaseDB.write_page(new PageId(runStart.pid + j), pg);
                    } catch (Exception e) {
                        status = FAIL;
    ...
    public void TestDBMethods(){
        PageId pgid = new PageId();
        pgid.pid = 0;
        /*Invalid page and I/O error already handled in the functions - readPage and writePage*/
        //assertEquals(,readPage(PageId pageno, Page apage),"");
        //writePage(PageId pageno, Page apage);
        //allocatePage(PageId start_page_num);

        //allocatePage(PageId start_page_num, int runsize);/*Negative run size is handled*/
        /*deallocatePage(PageId start_page_num, int run_size);
        deallocatePage(PageId start_page_num);
        addFileEntry(String fname, PageId start_page_num)
        deleteFileEntry(String fname)
        getFileEntry(String name)
        dumpSpaceMap()
        setBits(PageId start_page, int run_size, int bit)
        pinPage(PageId pageno, Page page, boolean emptyPage)
    ...
    public void afterTest(){
        ...
        System.out.print("\n" + "... ColumnDB tests ");
        //System.out.print(pass == OK ? "completely successfully" : "failed");
        System.out.print(".\n\n");
	...
	dbstatus = dbt.runTests();
        ...
        if (dbstatus != true) {
            System.err.println("Error encountered during column db tests:\n");
