...
public class BT implements GlobalConst {
	 ...
	 * 
	 * @param key
	 *            specify the key whose length will be calculated. Input parameter.
	 ...
	 * @param length
	 *            The length of (key, data) in byte array "from". Input parameter.
	...
	protected final static KeyDataEntry getEntryFromBytes(byte[] from, int offset, int length, int keyType,
		...
		try {
			...
			if (nodeType == NodeType.INDEX) {
				...
				TID tid = new TID();
				int numRids = Convert.getIntValue(offset + length - 8, from);
				n = 8 + numRids*8;
				tid.setNumRIDs(Convert.getIntValue(offset + length - 8, from));
				tid.setPosition(Convert.getIntValue(offset + length - 4, from));
				RID[] rids = new RID[numRids];
				for (int i = 0; i < numRids; i++) {
					RID rid = new RID();
					rid.pageNo = new PageId();
					rid.pageNo.pid = Convert.getIntValue(offset + length - 8 - ((i + 1) * 8) + 4, from);
					rid.slotNo = Convert.getIntValue(offset + length - 8 - ((i + 1) * 8) , from);
					rids[i] = rid;
				}
			...
			if (keyType == AttrType.attrInteger) {
				...
				key = new StringKey(Convert.getStringValue(offset, from, length - n));
			} else
				throw new KeyNotMatchException(null, "key types do not match");
	...
	protected final static byte[] getBytesFromEntry(KeyDataEntry entry)
		...
		try {int ridNum = 0;
			...
			if (entry.data instanceof IndexData)
				...
				ridNum =  ((LeafData) entry.data).getData().getNumRIDs();
				n += 8 + (8*ridNum);							
			}
			...
			if (entry.key instanceof IntegerKey) {
				...
				Convert.setStringValue(((StringKey) entry.key).getKey(), 0, data);
			} else
				throw new KeyNotMatchException(null, "key types do not match");
			...
			if (entry.data instanceof IndexData) {
				...
				for(int i =0 ;i< ridNum;i++) {
					Convert.setIntValue(((LeafData) entry.data).getData().getRecordIDs()[ridNum -i -1].pageNo.pid, m+(8*i)+4, data);
					Convert.setIntValue(((LeafData) entry.data).getData().getRecordIDs()[ridNum -i -1].slotNo, 8*i+(m), data);
					
				}
				Convert.setIntValue(((LeafData) entry.data).getData().getPosition(),m+8*ridNum+4, data);
				Convert.setIntValue(((LeafData) entry.data).getData().getNumRIDs(),m+(8*ridNum) , data);
