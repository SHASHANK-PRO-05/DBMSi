...
public class PredEval {
  ...
  public static boolean Eval(CondExpr[] p, Tuple t1, Tuple t2, AttrType[] in1,
    ...
    while (p[i] != null) {
      temp_ptr = p[i];
      while (temp_ptr != null) {
        val_type[0] = new AttrType(temp_ptr.type1.getAttrType());
        fld1 = 1;
        switch (temp_ptr.type1.getAttrType()) {
          case AttrType.attrInteger:
            value.setHdr((short) 1, val_type, null);
            value.setIntFld(1, temp_ptr.operand1.integer);
            tuple1 = value;
            comparison_type.setAttrType(AttrType.attrInteger);
            break;
          case AttrType.attrReal:
            value.setHdr((short) 1, val_type, null);
            value.setFloFld(1, temp_ptr.operand1.real);
            tuple1 = value;
            comparison_type.setAttrType(AttrType.attrReal);
            break;
          case AttrType.attrString:
            ...
            tuple1 = value;
            comparison_type.setAttrType(AttrType.attrString);
            break;
          case AttrType.attrSymbol:
            fld1 = temp_ptr.operand1.symbol.offset;
            if (temp_ptr.operand1.symbol.relation.key == RelSpec.outer) {
              tuple1 = t1;
              comparison_type.setAttrType(in1[fld1 - 1].getAttrType());
            } else {
              tuple1 = t2;
              comparison_type.setAttrType(in2[fld1 - 1].getAttrType());
            }
            break;
          default:
            break;
        ...
        val_type[0] = new AttrType(temp_ptr.type2.getAttrType());
        fld2 = 1;
        switch (temp_ptr.type2.getAttrType()) {
          case AttrType.attrInteger:
            value.setHdr((short) 1, val_type, null);
        ...
        try {
          ...
          throw new PredEvalException(unknowAttrType, "UnkonwnAttrType caught by PredEval");
        }
