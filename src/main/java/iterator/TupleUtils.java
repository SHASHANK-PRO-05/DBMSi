...
public class TupleUtils {
  ...
  public static short[] setup_op_tuple(Tuple Jtuple, AttrType[] res_attrs,
    ...
    for (i = 0; i < len_in1; i++)
      if (in1[i].getAttrType() == AttrType.attrString)
        sizesT1[i] = t1_str_sizes[count++];

    for (count = 0, i = 0; i < len_in2; i++)
      if (in2[i].getAttrType() == AttrType.attrString)
        sizesT2[i] = t2_str_sizes[count++];

    int n_strs = 0;
    for (i = 0; i < nOutFlds; i++) {
      if (proj_list[i].relation.key == RelSpec.outer)
        res_attrs[i] = new AttrType(in1[proj_list[i].offset - 1].getAttrType());
      else if (proj_list[i].relation.key == RelSpec.innerRel)
        res_attrs[i] = new AttrType(in2[proj_list[i].offset - 1].getAttrType());
    }

    // Now construct the res_str_sizes array.
    for (i = 0; i < nOutFlds; i++) {
      if (proj_list[i].relation.key == RelSpec.outer && in1[proj_list[i].offset - 1].getAttrType() == AttrType.attrString)
        n_strs++;
      else if (proj_list[i].relation.key == RelSpec.innerRel && in2[proj_list[i].offset - 1].getAttrType() == AttrType.attrString)
        n_strs++;
    }
  ...
  public static short[] setup_op_tuple(Tuple Jtuple, AttrType res_attrs[],
    ...
    for (i = 0; i < nOutFlds; i++) {
      if (proj_list[i].relation.key == RelSpec.outer
          && in1[proj_list[i].offset - 1].getAttrType() == AttrType.attrString)
        n_strs++;
    }
